...
def accessory_list_create(request):
    ...
    if request.method == "GET":
        queryset = Accessory.objects.select_related("category").all()
        ...
        serializer = AccessoryCreateSerializer(data=request.data)
        if serializer.is_valid():
            accessory = serializer.save()
            return Response(
                AccessoryDetailSerializer(accessory).data, status=status.HTTP_201_CREATED
            )
        return Response(
...
def accessory_detail(request, pk):
    ...
    try:
        accessory = Accessory.objects.select_related("category").get(pk=pk)
    except Accessory.DoesNotExist:
        return Response(
            {"error": "Accessory not found"}, status=status.HTTP_404_NOT_FOUND
        )

    if request.method == "GET":
        return Response(AccessoryDetailSerializer(accessory).data)

    elif request.method == "PUT":
        serializer = AccessoryCreateSerializer(accessory, data=request.data, partial=True)
        if serializer.is_valid():
            accessory = serializer.save()
            return Response(AccessoryDetailSerializer(accessory).data)
        return Response(
...
def update_stock(request, pk):
    ...
    accessory.stock = int(stock)
    accessory.status = "available" if accessory.stock > 0 else "unavailable"
    accessory.save()
