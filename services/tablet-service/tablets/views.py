...
from .serializers import (
    TabletSerializer,
    TabletDetailSerializer,
    TabletCreateSerializer,
)
...
@api_view(["GET", "POST"])
def tablet_list_create(request):
    ...
    if request.method == "GET":
        queryset = Tablet.objects.select_related("category").all()
        ...
        return paginator.get_paginated_response(
            TabletSerializer(page, many=True).data
        )

    elif request.method == "POST":
        serializer = TabletCreateSerializer(data=request.data)
        if serializer.is_valid():
            tablet = serializer.save()
            return Response(
                TabletDetailSerializer(tablet).data, status=status.HTTP_201_CREATED
            )
        return Response(
...
def tablet_detail(request, pk):
    ...
    try:
        tablet = Tablet.objects.select_related("category").get(pk=pk)
    except Tablet.DoesNotExist:
        return Response(
            {"error": "Tablet not found"}, status=status.HTTP_404_NOT_FOUND
        )

    if request.method == "GET":
        return Response(TabletDetailSerializer(tablet).data)

    elif request.method == "PUT":
        serializer = TabletCreateSerializer(tablet, data=request.data, partial=True)
        if serializer.is_valid():
            tablet = serializer.save()
            return Response(TabletDetailSerializer(tablet).data)
        return Response(
...
def update_stock(request, pk):
    ...
    try:
        tablet = Tablet.objects.get(pk=pk)
    except Tablet.DoesNotExist:
        return Response(
            {"error": "Tablet not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    tablet.stock = int(stock)
    tablet.status = "available" if tablet.stock > 0 else "unavailable"
    tablet.save()

    return Response(TabletSerializer(tablet).data)
