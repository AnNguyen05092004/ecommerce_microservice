"""BiLSTM (Bidirectional LSTM) based behavior profile classifier.

Uses event sequences (event_type + product_type per step) to classify
a user session into one of 5 behavior profiles. BiLSTM processes sequences
in both directions for richer context. Falls back gracefully when PyTorch
is unavailable — callers must handle None returns.
"""
...
# BiLSTM model based on event sequences (bidirectional context).
...
BILSTM_WEIGHTS_FILENAME = "behavior_bilstm.pt"
BILSTM_CONFIG_FILENAME = "behavior_bilstm_config.pkl"
BILSTM_METRICS_FILENAME = "behavior_bilstm_metrics.json"
...
if TORCH_AVAILABLE:
    ...
    class BehaviorBiLSTMNet(nn.Module):
        ...
        def forward(self, x):
            ...
            # For bidirectional: last layer = hidden[-2:] (forward, backward)
...
# Import torch for the forward pass
if TORCH_AVAILABLE:
    import torch as _torch_for_cat  # noqa: F401
...
def train_behavior_bilstm(sequence_path: Path, artifact_dir: Path, epochs: int = 25):
    """Train BiLSTM on labeled behavior sequences from behavior_sequences.json.

    Returns metrics dict. Always writes BILSTM_METRICS_FILENAME so callers can
    ...
    if not TORCH_AVAILABLE:
        metrics = {
            ...
            "note": "Install torch to enable BiLSTM training. Falling back to MLP.",
        }
        (artifact_dir / BILSTM_METRICS_FILENAME).write_text(
    ...
    model = BehaviorBiLSTMNet(**config)
    ...
    torch.save(model.state_dict(), artifact_dir / BILSTM_WEIGHTS_FILENAME)
    with (artifact_dir / BILSTM_CONFIG_FILENAME).open("wb") as fh:
    ...
    (artifact_dir / BILSTM_METRICS_FILENAME).write_text(json.dumps(metrics, indent=2))
    return metrics
...
def infer_behavior_bilstm(events, artifact_dir: Path):
    """Classify behavior profile from an event sequence.
    ...
    """
    weights_path = artifact_dir / BILSTM_WEIGHTS_FILENAME
    config_path = artifact_dir / BILSTM_CONFIG_FILENAME
