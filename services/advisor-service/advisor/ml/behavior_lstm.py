"""LSTM-based behavior profile classifier.
...
# LSTM model based on event sequences.
...
LSTM_WEIGHTS_FILENAME = "behavior_lstm.pt"
LSTM_CONFIG_FILENAME = "behavior_lstm_config.pkl"
LSTM_METRICS_FILENAME = "behavior_lstm_metrics.json"
...
if TORCH_AVAILABLE:
    ...
    class BehaviorLSTMNet(nn.Module):
        def __init__(
            self,
...
def train_behavior_lstm(sequence_path: Path, artifact_dir: Path, epochs: int = 25):
    """Train LSTM on labeled behavior sequences from behavior_sequences.json.

    Returns metrics dict. Always writes LSTM_METRICS_FILENAME so callers can
    ...
    if not TORCH_AVAILABLE:
        metrics = {
            ...
            "note": "Install torch to enable LSTM training. Falling back to MLP.",
        }
        (artifact_dir / LSTM_METRICS_FILENAME).write_text(json.dumps(metrics, indent=2))
        return metrics
    ...
    torch.save(model.state_dict(), artifact_dir / LSTM_WEIGHTS_FILENAME)
    with (artifact_dir / LSTM_CONFIG_FILENAME).open("wb") as fh:
...
def infer_behavior_lstm(events, artifact_dir: Path):
    ...
    """
    weights_path = artifact_dir / LSTM_WEIGHTS_FILENAME
    config_path = artifact_dir / LSTM_CONFIG_FILENAME
