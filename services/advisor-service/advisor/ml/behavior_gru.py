"""GRU-based behavior profile classifier.
...
GRU_WEIGHTS_FILENAME = "behavior_gru.pt"
GRU_CONFIG_FILENAME = "behavior_gru_config.pkl"
GRU_METRICS_FILENAME = "behavior_gru_metrics.json"
...
if TORCH_AVAILABLE:
    ...
    class BehaviorGRUNet(nn.Module):
        def __init__(
            self,
...
def train_behavior_gru(sequence_path: Path, artifact_dir: Path, epochs: int = 25):
    """Train GRU on labeled behavior sequences from behavior_sequences.json.

    Returns metrics dict. Always writes GRU_METRICS_FILENAME so callers can
    ...
    if not TORCH_AVAILABLE:
        metrics = {
            ...
            "note": "Install torch to enable GRU training. Falling back to MLP.",
        }
        (artifact_dir / GRU_METRICS_FILENAME).write_text(json.dumps(metrics, indent=2))
        return metrics
    ...
    torch.save(model.state_dict(), artifact_dir / GRU_WEIGHTS_FILENAME)
    with (artifact_dir / GRU_CONFIG_FILENAME).open("wb") as fh:
...
def infer_behavior_gru(events, artifact_dir: Path):
    ...
    """
    weights_path = artifact_dir / GRU_WEIGHTS_FILENAME
    config_path = artifact_dir / GRU_CONFIG_FILENAME
