...
def computer_list_create(request):
    ...
    if request.method == "GET":
        ...
        return paginator.get_paginated_response(
            ComputerSerializer(page, many=True).data
        )
        ...
        if serializer.is_valid():
            ...
            return Response(
                ComputerDetailSerializer(computer).data, status=status.HTTP_201_CREATED
            )
        return Response(
...
def computer_detail(request, pk):
    ...
    try:
        computer = Computer.objects.select_related("category").get(pk=pk)
    ...
    if request.method == "GET":
        return Response(ComputerDetailSerializer(computer).data)

    elif request.method == "PUT":
        serializer = ComputerCreateSerializer(computer, data=request.data, partial=True)
        if serializer.is_valid():
            computer = serializer.save()
            return Response(ComputerDetailSerializer(computer).data)
        return Response(
...
def update_stock(request, pk):
    ...
    computer.stock = int(stock)
    computer.status = "available" if computer.stock > 0 else "unavailable"
    computer.save()
