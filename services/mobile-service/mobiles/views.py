...
def mobile_list_create(request):
    ...
    if request.method == "GET":
        queryset = Mobile.objects.select_related("category").all()
        ...
        return paginator.get_paginated_response(MobileSerializer(page, many=True).data)
        ...
        if serializer.is_valid():
            ...
            return Response(
                MobileDetailSerializer(mobile).data, status=status.HTTP_201_CREATED
            )
        return Response(
...
def mobile_detail(request, pk):
    ...
    try:
        mobile = Mobile.objects.select_related("category").get(pk=pk)
    except Mobile.DoesNotExist:
        return Response({"error": "Mobile not found"}, status=status.HTTP_404_NOT_FOUND)

    if request.method == "GET":
        return Response(MobileDetailSerializer(mobile).data)
...
def update_stock(request, pk):
    ...
    mobile.stock = int(stock)
    mobile.status = "available" if mobile.stock > 0 else "unavailable"
    mobile.save()
