...
def category_list_create(request):
    if request.method == "GET":
        return Response(
            CategorySerializer(MobileCategory.objects.all(), many=True).data
        )
        ...
        if serializer.is_valid():
            return Response(
                CategorySerializer(serializer.save()).data,
...
def category_detail(request, pk):
    ...
    except MobileCategory.DoesNotExist:
        return Response(
    ...
    if request.method == "GET":
        ...
        if serializer.is_valid():
            ...
            return Response(CategorySerializer(category).data)
        return Response(
            {"error": "Validation failed"}, status=status.HTTP_400_BAD_REQUEST
        )
